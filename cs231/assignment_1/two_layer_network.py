...
def dataprepocessing(X_train, X_val, X_test):  # 对数据进行预处理，实现数据的零均值
...
def layer(inputs, insize, outsize, activation_function=None):
    W = tf.Variable(tf.random_normal([insize, outsize], stddev=0.001))
...
X_train, X_val, X_test = dataprepocessing(X_train, X_val, X_test)
...
learning_rate = tf.train.exponential_decay(initial_learning_rate, global_step=global_step, decay_steps=1000,
                                           decay_rate=0.9)
